...
for i in range(Bs):
    ...
    one = tf.cond(tf.less(tf.shape(one)[0], Ns * 2 + 1),
                 lambda: tf.expand_dims(tf.range(Ns * 2 + 1)[2::2], 0),
                 lambda: tf.sort(tf.reshape(one[: Ns * 2 + 1], [1, Ns * 2 + 1]))[:, 2::2])
    ...
    for j in range(size_split):
        ...
        if j < size_split -1: #exclude the last split
            if j % 2 == rand_check: #label 2 for insertion
                ...
                inputs = tf.concat([inputs, tf.random.uniform([1], 0, Vs, tf.int32)], 0)
...
sess = tf.Session()
print(sess.run(size_splits))

a, b, c, d = sess.run([new_inputs_tf, new_labels_tf, mask, inputs_tf])
print(a)
print(b)
print(c)
print(d)
